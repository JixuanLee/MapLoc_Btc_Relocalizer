...
/**
 * @brief 距离判断2坨坐标系已经对齐的点云是否匹配
 * @param cloud1 当前帧点云
 * @param cloud2 匹配到的过去帧点云
 * @param dis_threshold 距离阈值以判断重合 or not
 * @return double 重合点的占比
 */
double calc_overlap(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud1,
  ...
  //           match_num
  //           << std::endl;
  double overlap = 2 * match_num * point_kip / (cloud1->size() + cloud2->size());
  return overlap;
}
